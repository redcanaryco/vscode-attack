...
function buildCompletionItem(label: string, software: Software): vscode.CompletionItem {
    ...
    let insertionText: string = buildInsertionText(software);
...
export async function init(attackData: AttackMap): Promise<Array<Software>> {
    return new Promise((resolve) => {
        ...
        softwares = attackData.objects.filter((item: AttackObject) => {
            ...
            let aliases: Array<string> = item.x_mitre_aliases !== undefined ? item.aliases : new Array<string>();
...
export function register(filters: vscode.DocumentSelector, tools: Array<Software>): Array<vscode.Disposable> {
    // hover provider
    let softwareHovers: SoftwareHoverProvider = new SoftwareHoverProvider();
    let softwareHoverDisposable: vscode.Disposable = vscode.languages.registerHoverProvider(filters, softwareHovers);
    softwareHovers.software = tools;
    // completion provider
    let softwareCompletions: SoftwareCompletionProvider = new SoftwareCompletionProvider();
    softwareCompletions.software = tools;
    let softwareCompletionDisposable: vscode.Disposable = vscode.languages.registerCompletionItemProvider(filters, softwareCompletions);
		...
		try {
			return new Promise((resolve) => {
				...
				if (hoverRange !== undefined) {
                    ...
                    let currentSoftware: Software | undefined = this.software.find((g: Software) => { return g.id === hoverTerm; });
            ...
            return new Promise((resolve) => {
                ...
                if (completionRange === undefined) {
                    ...
                    if (completionTerm.length >= minTermLength) {
                        ...
                        let software: Software | undefined = this.software.find((g: Software) => { return g.id === completionTerm.toUpperCase(); });
                        if (software !== undefined) {
                            ...
                            let possibleSoftware: Array<Software> | undefined = this.software.filter((s: Software) => {
    ...
    public resolveCompletionItem(item: vscode.CompletionItem, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CompletionItem> {
        try {
            return new Promise((resolve) => {
                ...
                let software: Software | undefined = this.software.find((g: Software) => {
