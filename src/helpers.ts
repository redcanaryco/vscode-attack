...
export let output: vscode.OutputChannel = vscode.window.createOutputChannel("MITRE ATT&CK");
// should match T1059.001
export const techniqueRegex: RegExp = /T\d{4}([\.\/]\d{3})?/;
// should match TA0001
export const tacticRegex: RegExp = /TA\d{4}/;
// should match G0073
export const groupRegex: RegExp = /G\d{4}/;
// should match S0363
export const softwareRegex: RegExp = /S\d{4}/;
// should match M1036
export const mitigationRegex: RegExp = /M\d{4}/;

// minimum completion item term length
// everything under this will only show the technique provider's results
export const minTermLength: number = 5;
...
export function isAttackMapNewer(prospective: AttackMap, familiar: AttackMap): boolean {
    ...
    let prospectiveTime: string|undefined = getModifiedTime(prospective);
    let familiarTime: string|undefined = getModifiedTime(familiar);
    ...
    if (prospectiveTime !== undefined && familiarTime !== undefined) {
        let prospectiveDate: Date = new Date(prospectiveTime);
        let familiarDate: Date = new Date(familiarTime);
...
export function downloadAttackMap(storageDir: string, version: string): Promise<string> {
    ...
    return new Promise<string>(async function (resolve, reject) {
        let availableVersions: Array<string> = await getVersions();
        if (availableVersions.includes(version)) {
            ...
            const url: string = `https://raw.githubusercontent.com/mitre/cti/ATT%26CK-v${version}/enterprise-attack/enterprise-attack.json`;
...
export async function downloadLatestAttackMap(storageDir: string): Promise<AttackMap|undefined> {
    ...
    let version: string = `${availableVersions.sort()[availableVersions.length - 1]}`;
    let downloadedData: string = await downloadAttackMap(storageDir, version);
...
export function getLatestCacheVersion(cacheDir: string): Promise<string|undefined> {
    const pattern: string = 'enterprise-attack\..*\.json';
    return new Promise<string|undefined>((resolve, reject) => {
        ...
        fs.readdir(cacheDir, (err, files) => {
            if (err) {
                ...
                let latestVersionName: string|undefined = files.filter((file: string) => {
...
export function getModifiedTime(mapping: AttackMap): string|undefined {
    let mapMatrices: Array<Matrix> = mapping.objects?.filter((obj: AttackObject) => {
...
*/
export function getVersions(prefix: string = 'ATT&CK-v'): Promise<Array<string>> {
    ...
    const url: string = 'https://api.github.com/repos/mitre/cti/git/refs/tags';
    let downloadedData: string = '';
    const options = {
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(url, options, (res: IncomingMessage) => {
            ...
            res.on('end', () => {
                let tags: Array<Tag> = JSON.parse(downloadedData) as Array<Tag>;
