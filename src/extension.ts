...
const Providers = {
	...
	pushAll: function (list: Array<vscode.Disposable>) {
...
let currentProviders = Object.create(Providers);
...
export async function cacheData(storageDir: string): Promise<AttackMap|undefined> {
	return new Promise(async (resolve) => {
		...
		if (!fs.existsSync(storageDir)) {
			...
			let cachedPath: string|undefined = await helpers.getLatestCacheVersion(storageDir);
			if (cachedPath === undefined) {
				...
				let cachedVersion = path.basename(cachedPath).replace('enterprise-attack.', '').replace('.json', '');
				let availableVersions: Array<string> = await helpers.getVersions();
				let onlineVersion: string = `${availableVersions.sort()[availableVersions.length - 1]}`;
				if (cachedVersion < onlineVersion) {
					...
					let cachedData: string = fs.readFileSync(cachedPath, {encoding: 'utf8'});
...
async function createStatusBar(storageDir: string): Promise<vscode.StatusBarItem|undefined> {
	...
	let cachedPath: string|undefined = await helpers.getLatestCacheVersion(storageDir);
	if (cachedPath !== undefined) {
		let version = path.basename(cachedPath).replace('enterprise-attack.', '').replace('.json', '');
		let itemText: string = `ATT&CK v${version}`;
...
*/
function toggleStatusBar(statusBarItem: vscode.StatusBarItem, editor: vscode.TextEditor|undefined) {
	let applicableFiles: vscode.DocumentSelector|undefined = vscode.workspace.getConfiguration(configSection).get('applicableFiles');
...
*/
export function registerFeatures(techniques: Array<Technique>, tactics: Array<Tactic>, groups: Array<Group>, software: Array<Software>, mitigations: Array<Mitigation>) {
	...
	let applicableFiles: vscode.DocumentSelector|undefined = configuration.get('applicableFiles');
...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (attackData === undefined) {
		...
		// window
		let statusBarItem: vscode.StatusBarItem|undefined = await createStatusBar(context.globalStorageUri.fsPath);
